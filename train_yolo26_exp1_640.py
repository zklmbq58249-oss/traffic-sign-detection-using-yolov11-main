"""EXP-1: retrain YOLO26n at 640x640 and evaluate on the test split.

This experiment keeps the baseline dataset, seed, epoch count, batch size,
device, and worker count unchanged. The only training change is imgsz=640.
The generated training and test outputs use a dedicated run name so the
existing 416px baseline and previous test reports are not overwritten.
"""
...
# EXP-1 settings. Keep these fixed when comparing against the 416px baseline.
MODEL = "yolo26n.pt"
...
WORKERS = 4
PROJECT = "runs/yolo26"
NAME = "yolo26n_traffic_signs_exp1_640"
...
def main() -> int:
    ...
    test_name = f"{NAME}_test_e2e_true"
    print("Evaluating best.pt on the test split")
